// Cache bust: 2026-05-23-v6 - Live खाता rebrand + domain migration to livekhaata.app
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function(clientList) {
      for (let i = 0; i < clientList.length; i++) {
        if ((clientList[i].url.includes('livekhaata') || clientList[i].url.includes('hisaabkitab')) && 'focus' in clientList[i]) {
    ...
    Promise.all([
      ...
      return self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function(clientList) {
        clientList.forEach(function(client) {
          client.postMessage({ type: 'SW_UPDATED', version: '2026-05-23-v6' });
        });
      });
    })
  );
